...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      ...
      onOptionMouseover(e, option, index) {
        this.selectable(option) ? this.typeAheadPointer = index : null
      },

      onOptionMousedown(e, option, index) {
        e.preventDefault(e)
        e.stopPropagation(e)
    ...
    computed: {
       ...
       */
      searchEl () {
        return this.$el.querySelector('[data-vs-ref="search"]')
      },
    ...
    render() {
      return this.$scopedSlots.default({
        ...
        state: {
          ...
          searching: this.searching && !this.noDrop,
          searchable: this.searchable && !this.noDrop,
          unsearchable: !this.searchable,
          loading: this.mutableLoading,
          disabled: this.disabled,
          selectedValue: this.selectedValue,
          highlightedIndex: this.typeAheadPointer
        },
        ...
        api: {
          ...
          normalizeOptionForSlot: this.normalizeOptionForSlot,
        ...
        refs: {
          combobox: {
            attributes: {
              'data-vs-ref': 'combobox',
              'id': `vs${this.uid}__combobox`,            
              'role': "combobox",
          ...
          /**
           * Select options containers
           */
          listbox: {
            attributes: {
              id: `vs${this.uid}__listbox`,
              key: `vs${this.uid}__listbox`, 
          ...
          /**
           * Options provided up must already containt this?
           */
          optionGetter: (option, index) => ({
            attributes: {
              role: "option",          
              key: this.getOptionKey(option),
              id: `vs${this.uid}__option-${index}`,
              // class="vs__dropdown-option"
              // :class="{ 'vs__dropdown-option--selected': isOptionSelected(option), 'vs__dropdown-option--highlight': index === typeAheadPointer, 'vs__dropdown-option--disabled': !selectable(option) }"
              'aria-selected': index === this.typeAheadPointer ? true : null
            },
            events: {
              //mouseover: o
              mouseover: (e) => this.onOptionMouseover(e, option, index),
              mousedown: (e) => this.onOptionMousedown(e, option, index)
            }
          })
        }
      })
